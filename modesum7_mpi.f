...
c
	...
	double precision tbegin,tend,tend1,tbeginx,tendx,vpmax
...
c   Initialize MPI.
	...
	call MPI_INIT(ierr)
 	...
 	IF(ierr/=0) THEN
          WRITE(*,*) 'Error: Cannot start MPI!!!'
          CALL MPI_ABORT(MPI_COMM_WORLD,iskip,ierr)
	ENDIF

c   ncpu returns number of processes started.
c   myrank is the rank of each process, between 0 and ncpu-1.
...
c   Master process read input file.

        if(myrank.eq.0) call scrinp
	
	call MPI_BCAST(inname,nchmax,MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
	call MPI_BCAST(nst,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
	
	nloop=(nst-1)/ncpu
	do 1000 iloop=0,nloop
	
	innamex=''
	jst=iloop*ncpu+myrank+1
	if(jst.gt.nst) goto 1000
	...
	nj=int(1.5d0*hdur/dt)+1
        ...
        write(*,'(a,i3,a,2x,f6.2,2x,f7.2,2x,f7.1)') 
     1    '    Rank ',myrank,'. Source coordinates (lat, long, depth): ',
     1    evtlat,evtlon,depths0
        write(*,'(a,i3,a,2x,f6.2,2x,f7.2,2x,f8.2)') 
     1	  '    Rank ',myrank,'. Station coordinates (lat, long, depth): ',
     2    stlat,stlon,stelv
...
c   receiver->source line measured clockwise. 
	...
	crs=dcos(phirs)
        ...
        write(*,'(a,i3,a,4(2x,f8.4))') '    Rank ',myrank,'. Distance and azimuths (deg): ',
     1	  delta,azim,bazim
...
c   Set moment tensor elements of extremely small absolute value to zero.
	...
	write(*,'(a,i3,a,6(1x,e10.3))') '    Rank ',myrank,'. Normalized moment tensor: ',
...
c   Find out lmax by reading the summary mode files.
	...
	do i=1,nfiles+nfilet+nfiler
	  ...
	  if(myrank.eq.0) write(*,'(/a,a)') '    Examining: ',modepath(1:k)//modefile(1:k1)
     1	    //'.'//amode
	...
	if(bpf(2).gt.0.d0) then
	  if(myrank.eq.0) write(*,'(/a,4(1x,f7.2))') 
...
c   l=1 to l=lmax1. 
        ...
        if(myrank.eq.0) write(*,'(/a,i6/)') '    Calculating the GLF up to l=',lmax1
        call glegendre(delta)

c   Loop over mode table files and conduct mode summation.

	if(myrank.eq.0) write(*,'(a,i3)') '    Starting mode summation. Number of mode files: ',
	...
	do it=0,nt0
	  t(it)=t1+dble(it)*dt
     	...
     	tbeginx=(rnorm*delta*pi/180.d0)/vpmax-dt
	if(tbeginx.lt.tbegin) tbeginx=tbegin
	tendx=tend
	if(delta.lt.90.d0) tendx=tend1+dt
	if(tendx.gt.t(nt0)) tendx=t(nt0)
	
	if(myrank.eq.0) write(*,'(a,2(2x,f8.3),a,2(2x,f8.3))') 
     1	  '    Summation time window (begin, end): ',
     1	  tbeginx,tendx,', in ',t(0),t(nt0)
...
c   individual modes.
	...
	if(myrank.eq.0) write(*,'(/a)') '    Computing time-independent mode-sum coefficients.'
...
c   Loop over time samples and sum the modes.
	...
	if(myrank.eq.0) write(*,'(/a/)') '    Computing the waveform time series.'
	mt=nt0/15
	nt1=0
	if(t(nt1).le.0.d0) nt1=1
	do it=nt1,nt0
	  if((t(it).ge.tbeginx).and.(t(it).le.tendx)) then
	    if((mod(it,mt).eq.0).and.(myrank.eq.0)) write(*,'(a,i7,a,i7)') 
     1	      '    Time step: ',it, ' out of ',nt0
            do k=1,nmodes
	      if(it.eq.(int(tbeginx)+1)) nmodest=nmodest+1
	      if(weight(k).ne.0.) then
	        if(it.eq.(int(tbeginx)+1)) nsumtot=nsumtot+1
...
c   to conform with the sign convention in SAC. 
	...
	if(myrank.eq.0) write(*,'(/a,i9,a,i9,a,i9)') '    Summation done. Total mode number: ',
     1	  nmodest,'. Repeated: ',nrept,'. Summed: ',nsumtot
	write(*,'(a,i3,a,a)') '    Rank ',myrank,'. Output filename: ',
     1	  opath(1:ko)//eventid(1:ke)//'.'//stat(1:k)//'.ASC'

c   Record the date and time at the beginning of the job.

        call date_and_time(datex,timex)
        if(myrank.eq.0) write(*,'(/a,a4,a,a2,a,a2,a,a2,a,a2,a,a4/)') 
...
c   assumes the units of dyne and cm, hence 7 is subtracted to convert to N and m.
	...
	write(*,'(a,i3,a,f5.2,a,3(1x,f7.2),2(1x,f6.2))') '    Rank ',myrank,'. Mw: ',
     1	  xm0,'. Fault-plane, tmshift and hdur: ',xm(1),xm(2),xm(3),tmshift,hdur
...
1	do i=1,3
	...
	write(*,'(a,i3,a,6(1x,e10.3))') '    Rank ',myrank,'. Moment tensor in GCS: ',
...
c   First calculate functions with m=-2 using recursive relation.
	  ...
	  if((mod(l,5000).eq.0).and.(myrank.eq.0)) write(*,'(a,i7)') '    Processing l=',l
...
c   First open mode table header file and read header
	...
	if(myrank.eq.0) write(*,'(/a,a)') '    Reading: ',
...
100     close(20)
	...
	do i=1,nmodes
	  ...
	  if(ffmax.lt.fx(i)) ffmax=fx(i)
        ...
        if(myrank.eq.0) write(*,'(a,1x,i9,2(1x,i6,1x,i6))')
     1    '    Mode number, min-max n and l in mode table:  ',
     2	  nmodes,nnmin,nnmax,llmin,llmax
 	if(myrank.eq.0) write(*,'(a,i9)') '    Number of repeated modes: ',nrep
	if(myrank.eq.0) write(*,'(a,2(2x,f8.6))') '    Min. and Max. frequencies (Hz): ',
...
2	continue
	if(jcom.eq.2) licb=0
	if(jcom.eq.2) lcmb=1
	if(myrank.eq.0) write(*,'(/a,3(2x,i6))') 
...
55      continue         
	...
	if(rob.lt.rnorm) then
	  if(myrank.eq.0) write(*,'(a,f9.4)') '    Radius of sea floor: ',rob/1.d3
          if(depths0.le.((rnorm-rob)/1.d3)) then
            if(myrank.eq.0) write(*,'(/a,3x,f7.3,a)') 
     ...
     1      '    *** Receiver depth in the ocean: ',real(depthr),'km'
	  ...
	  if(myrank.eq.0) write(*,'(a)') '    This is a continental model.'
        endif
...
c   elevation stelv is given in m (upward from 0 at surface).
	...
	write(*,'(a,i3,a,2(4x,f9.1))') '    Rank ',myrank,'. Final source and station radii:   ',
     1	  rs0,rr0
	...
	if((jcom.eq.3).or.(jcom.eq.1)) goto 151
        ...
        if((rs0.lt.rmod(1)).or.(rr0.lt.rmod(1))) then
          nmodes=0
          write(*,'(a,i3,a,3(1x,f10.5))') 
     1	    '    Rank ',myrank,'. Source or receiver below the CMB. Toroidal modes ignored.',
