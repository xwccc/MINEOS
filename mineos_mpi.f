...
c   This MPI code computes a mode catalogue down to 3.3 seconds
...
c     -- Separate different tasks into individual subroutines
	...
	integer, parameter :: IOUT=35,IIN=36,rootid=0,tag1=200
...
c   Initialize MPI.
	...
	call MPI_INIT(ier)
 	...
 	IF (ier/=0) THEN
          ...
          CALL MPI_ABORT(MPI_COMM_WORLD, skip, ier)
	END IF
...
c   Sizeprocs returns number of processes started.
c   myrank is the rank of each process, between 0 and sizeprocs-1.
...
c   relation and further increase it by 10%.
	...
	call MPI_BCAST(pmax,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
...
c   Further reduce minimum frequency from Jean's prediction by a factor of 10.

	  fmhzmin1=fmhzmin1/10.d0
	  if(fmhzmin1.gt.fmhzmin0) fmhzmin0=fmhzmin1
	  IF(fmhzmin0.ge.fmhzmax) EXIT 	  
         
c   Determine frequency range for pheroidal modes.

	  CALL frange(l,fmhzmin0,fmhzmin)

c   Run mineos for spheroidal modes.
...
c   Determine frequency range for toroidal modes.

	  CALL frange(ll,fmhzmin0,fmhzmin)
	  
c   Run mineos for toroidal modes.
...
c   Determine frequency range for radial modes.

          CALL frange(0,fmhzmin0,fmhzmin)
	  
c   Run mineos for radial modes.
...
c   Synchronize all the processes to make sure everybody has finished.

	call MPI_BARRIER(MPI_COMM_WORLD,ier)
        time=time+MPI_WTIME()
        IF(myrank==0) WRITE(*,*) 'Elapsed time=', time        

c   Stop all the MPI processes, and exit
...
c   Run mineos.
          ...
          write(6,'(a,i3,a,i5,a,f8.3,a,f8.3)')
     1      '  CPU Rank ',myrank,' running mineos for '//TRIM(mtype)//
     2	    ' for degree      l=',l,'. fmhzmin=',fmhzmin,', fmhzmax=',fmhzmax
          call flush(6)
...
c    Run checkmode
          ...
          write(6,'(a,i3,a,i5)')
     1      '  CPU Rank ',myrank,' running checkmode for '//
     2	    TRIM(mtype)//' for degree l=',l
          call flush(6)
